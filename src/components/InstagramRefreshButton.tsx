...
export default function InstagramRefreshButton() {
  ...
  async function handleClick() {
    ...
    setMessage(null);
    try {
      const res = await fetch("/api/refresh-instagram", { method: "POST" });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error ?? "Refresh failed");
      setMessage(
        `Updated ${json.updated} of ${json.total} Instagram posts` +
          (json.notFound ? ` (${json.notFound} not matched by URL)` : "")
      );
      setStatus("idle");
  ...
  return (
    <div className="mb-4 flex items-center gap-3">
      ...
      >
        {status === "loading" ? "Refreshing…" : "↻ Refresh Instagram Stats"}
      </button>
      {message && (
